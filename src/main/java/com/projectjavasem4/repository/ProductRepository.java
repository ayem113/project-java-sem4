...
public interface ProductRepository extends JpaRepository<ProductEntity, Long> {
    ...
    @Query(nativeQuery =true,value = "SELECT * FROM product as e WHERE e.id IN :names")   // 3. Spring JPA In cause using native query
    ...
    @Query(nativeQuery =true,value = "SELECT * FROM product u WHERE u.id IN :ids")    //SELECT u FROM product u WHERE u.id IN :ids sqlserver
